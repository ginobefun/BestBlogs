...
BestBlogs.dev 是 AI 驱动的内容聚合与阅读平台。每天从 400+ RSS 订阅源自动抓取文章、播客、视频与推文，经大语言模型六维评分、摘要、翻译后，按每位用户的兴趣标签生成 **Daily Brief**，让阅读更高效、学习更轻松。
...
| **Daily Brief** | 每日个性化摘要（文字 / 音频 / 邮件），Pro 专属深度版 + 公开版 |
...
**网站订阅**：[bestblogs.dev/#subscribe](https://www.bestblogs.dev/#subscribe)
...
网站内容来源于 400+ RSS 订阅源，可导入到任意 RSS 阅读器：
...
**鉴权**：Header `X-API-KEY: <your_key>`（在 [Settings](https://bestblogs.dev/settings/api-keys) 生成，或运行 `bestblogs auth login`）

| 模块 | 说明 |
|---|---|
| [openapi/01-auth.md](./openapi/01-auth.md) | 认证与身份，公共规范 |
| [openapi/02-intake.md](./openapi/02-intake.md) | 兴趣画像建立，冷启动引导 |
| [openapi/03-discover.md](./openapi/03-discover.md) | 内容发现：Daily Brief / For You / 搜索 |
| [openapi/04-read.md](./openapi/04-read.md) | 内容阅读：全文 / Markdown / AI 边注 |
...
详细双语 Changelog 见 [changelog/](./changelog/)（从 v2.0.5 起）。
...
**3. AI 深度分析**：大语言模型（GPT-4o）生成六维评分 + 摘要 + 关键观点 + 金句 + 标签

**4. 多语言翻译**：识别专业术语 → 初译 → 检查 → 意译优化，支持中英双语

**5. 个性化推荐**：六维兴趣标签匹配 + Daily Brief 智能编排
