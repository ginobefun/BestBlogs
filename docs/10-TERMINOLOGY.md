...
|---|---|---|---|---|
| 世界杯特刊 | World Cup Special | 围绕世界杯赛事组织的独立活动页，包含比赛内容、Timeline、竞猜和排行榜 | 用户可见、产品文档 | 不写成主产品入口；英文不用 World Cup Betting |
| 金球币 | Golden Coin / GC | 世界杯特刊内的虚拟积分，用于娱乐竞猜和排行榜 | 用户可见、内部实现 | 不可充值、提现或兑换现金 |
| 竞猜 | Prediction Pick | 用户对比赛胜平负或比分结果的娱乐性选择 | 用户可见 | 可使用「下注」作为场景化口语，但必须配合合规说明 |
| 赔率 | Odds | 当前特刊内的虚拟全球参考倍率，用于计算娱乐竞猜返还 | 用户可见、产品文档 | 不接入真实投注平台，不承诺收益 |
| 普神 | Prophet | 世界杯特刊中的 AI 主播 / NPC，负责预测、复盘和公开下注单 | 用户可见 | 可正式入榜，但表达上避免赌场庄家感 |
| 屠龙者 | Dragon Slayer | 在排行榜中超过普神或战胜普神的荣誉身份 | 用户可见 | 可有仪式感，不做满屏炫光 |
