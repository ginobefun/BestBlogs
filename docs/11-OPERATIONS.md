...
|---|---|---|---|---|
| `FEATURE_WORLDCUP_SPECIAL_ENABLED` | `false` | 世界杯特刊总开关（#1122） | `WorldCupController` 公开读接口统一返回 `FEATURE_DISABLED`；用户竞猜、签到/任务、排行榜刷新、普神单生成、World Cup Job 全部 fail-closed；前端 `/worldcup` 与 `/worldcup/matches/*` 服务端 gate 404 | Admin 参数配置页 `worldcup` 分组置 `false` → 调 `POST /api/admin/parameter/refreshCache` 刷新缓存 → 前端硬刷新 |
| `WORLDCUP_COMPLIANCE_APPROVED` | `false` | 世界杯特刊合规批准门禁（#1122） | 即使公开页/竞猜开关为 `true`，公开读 API 与竞猜写路径仍返回 `FEATURE_DISABLED`；前端 feature flags 中 `worldCupPublicPage/worldCupBetting` 会保持 `false` | 仅在合规说明、虚拟积分边界、运营口径确认后置 `true`；事故或法务复核时优先置回 `false`，比单独关公开页/竞猜更彻底 |
| `WORLDCUP_PUBLIC_PAGE_ENABLED` | `false` | 世界杯特刊公开页面开关（#1122） | 在 `FEATURE_WORLDCUP_SPECIAL_ENABLED=true` 且 `WORLDCUP_COMPLIANCE_APPROVED=true` 时控制公开页与公开读 API；后台 seed/settle/leaderboard 操作仍可用于准备数据；客户端生产环境不 fallback mock | Admin 置 `false` 回滚公开访问；确认无误后可保留数据并再次置 `true` 恢复 |
| `WORLDCUP_BETTING_ENABLED` | `false` | 世界杯特刊竞猜写路径开关（#1122） | 在 `FEATURE_WORLDCUP_SPECIAL_ENABLED=true` 且 `WORLDCUP_COMPLIANCE_APPROVED=true` 时允许 `POST /api/users/me/worldcup/bets`；关闭时前端隐藏竞猜按钮；已锁定竞猜不撤销，后续可由 admin settle 或 void | 上线前保持 `false`；小流量验证通过后置 `true`。异常时置回 `false`，必要时用 `POST /api/admin/worldcup/matches/settle` 且 body 传 `{matchId, voidMatch:true}` 退还 stake |
| `WORLDCUP_COIN_TASK_ENABLED` | `false` | 世界杯特刊金球币任务/签到开关（#1122） | `check-in` 与任务领取写路径返回 `FEATURE_DISABLED`；前端隐藏签到按钮；已有金币和流水不回滚 | Admin 置 `false` 暂停发币；若误发，保留 ledger 审计并通过后续补偿/排行榜规则处理 |
| `WORLDCUP_LEADERBOARD_ENABLED` / `WORLDCUP_PROPHET_BET_ENABLED` / `WORLDCUP_ODDS_SYNC_ENABLED` | `false` | 世界杯排行榜、普神公开单、赔率同步开关（#1122） | 对应 Job 或运营入口跳过执行；公开页仍可展示已发布静态/手工数据 | Admin 分别置 `false` 停止刷新；数据异常时先停 Job，再用后台重新 seed/refresh 或清理对应 `bb_worldcup_*` 集合 |
| `WORLDCUP_DAILY_FLOOR_GC` / `WORLDCUP_ACTIVE_BONUS_GC` / `WORLDCUP_PRO_BONUS_GC` / `WORLDCUP_NEW_USER_GC` | `500` / `200` / `300` / `1000` | 金球币发放参数（#1122） | 调整签到补足目标、活跃奖励、Pro 加成和新人礼包数值；不影响历史 ledger | Admin 参数配置页即时调整；事故期优先关闭 `WORLDCUP_COIN_TASK_ENABLED`，再核对 `bb_worldcup_coin_ledger` |
| `WORLDCUP_DAILY_TASK_ENABLED` | `false` | 世界杯每日任务轮换（#1147） | DAILY 任务不进入任务列表，进度记录静默跳过；当日已分配记录保留在 `bb_worldcup_daily_task_assign` 供排查 | Admin 置 `false` 即时止血；重开后用户当日组合不变（分配已持久化） |
| `WORLDCUP_DAILY_TASK_COUNT` | `3` | 每人每日任务抽取数量（#1147） | 调整只影响下一次惰性生成；当日已生成的分配不变 | Admin 即时调整（合法值 1-12） |
| `WORLDCUP_TREASURE_ENABLED` | `false` | 世界杯寻宝随机掉落（#1147） | `treasure/roll` 静默返回未中，前端 `worldCupTreasure` flag 同步关闭、页面不再发请求 | Admin 置 `false` 无感止血；摇号记录与 `LUCKY_DROP` 流水保留审计 |
| `WORLDCUP_TREASURE_DROP_RATE_PERCENT` / `WORLDCUP_TREASURE_DAILY_LIMIT` | `20` / `3` | 寻宝概率与每人每日中奖上限（#1147） | 调整下次摇号即时生效；不影响已摇记录与已中次数 | 轻度通胀先降概率/上限，重度直接关 `WORLDCUP_TREASURE_ENABLED`；按 `worldcup_coin_granted(type=LUCKY_DROP)` 观察发放结构 |
| `WORLDCUP_MIN_BET_GC` / `WORLDCUP_MAX_PAYOUT_GC` / `WORLDCUP_BET_LOCK_BEFORE_MINUTES` | `100` / `100000` / `0` | 竞猜风控参数（#1122） | 控制最小下注、单注返还上限、开赛前封盘分钟；开放赔率竞猜按新幂等 key 独立锁定，历史已锁定竞猜不重算 | Admin 调整后新下注即时生效；紧急封盘可把 lock minutes 调大或直接关闭 `WORLDCUP_BETTING_ENABLED` |
| `WORLDCUP_DEFAULT_TAB_SWITCH_HOURS` / `WORLDCUP_ODDS_PROVIDER` / `WORLDCUP_TIMELINE_TAGS` / `WORLDCUP_COMPLIANCE_TEXT` | `4` / `MANUAL` / `""` / 见 `ConfigKey` 默认免责声明 | 特刊展示、赔率来源、Timeline 标签、合规文案（#1122） | 控制推荐 Tab、赔率来源展示、Timeline 分类和合规说明；字符串可清空回退默认 | Admin 参数配置页调整；合规文案修改后需产品/法务确认再发布 |
...
| `CONTENT_REVIEW_TELEGRAM_ENABLED` / `_BOT_TOKEN` / `_CHAT_ID` / `_WEBHOOK_SECRET` / `_WORLDCUP_SCORE_THRESHOLD` | `false` / `""` / `""` / `""` / `85` | 高分内容评审推送 Telegram 审核群（Issue #1086 · **专用 bot，与告警 / 早报审批 bot 隔离**）。即时：普通内容 ≥`HIGH_SCORE_NOTIFY_THRESHOLD` 时发 Bark + Telegram；命中 `entity-fifa-world-cup` 标签时 Telegram 可按 `_WORLDCUP_SCORE_THRESHOLD` 低阈值推送（不影响 Bark）。定时：`ContentReviewDigestJob` 每小时 :20 汇总 COMPLETED + qualified=null + 普通高分或世界杯低阈值内容。卡片带 6 个 inline 按钮（+3/+2/-2/-3/标记精选/标记非精选），显示来源、类型、分类、语言；世界杯待评审卡片使用足球图标；点击经 `POST /api/admin/webhook/telegram-review` 回调 `RawResourceMgrService` 执行评审，并编辑原消息标题为「✅ 高分已评审」。`_BOT_TOKEN` / `_WEBHOOK_SECRET` 是 sensitive，admin UI 走 `maskSensitive`+sentinel | `_ENABLED=false` → 即时与定时同时停止（监听器 / Job 头部检查跳过）；`_BOT_TOKEN` / `_CHAT_ID` 空 → `ContentReviewTelegramService` fail-soft 跳过；`_WEBHOOK_SECRET` 空 → webhook 拒绝所有回调（fail-closed）；`_WORLDCUP_SCORE_THRESHOLD` 调低会增加世界杯内容审核量 | Admin 参数配置页 `运维与集成 → 高分内容评审推送` 分组置 `false` 紧急停用；参数缓存 ≤ 5 min 失效。**初次启用 SOP**：① @BotFather 建 bot 并拉入审核群 → ② 填 `_BOT_TOKEN` / `_CHAT_ID` / `_WEBHOOK_SECRET`（`openssl rand -base64 32`）→ ③ 调 Telegram `setWebhook` 指向 `https://api.bestblogs.dev/api/admin/webhook/telegram-review` 并带 `secret_token`（参考早报审批 bot 的 setWebhook 步骤）→ ④ 置 `_ENABLED=true`。`JobHealthCheckJob.JOB_OVERRIDES` 已登记 `ContentReviewDigestJob` 130 min 阈值 |
...
- [ ] **（升级到 v2.5.0+ 世界杯特刊时）**重跑 `mongosh "$SPRING_DATA_MONGODB_URI" deploy/init/00_create_indexes.js`：脚本幂等，会创建/确认世界杯特刊任务与寻宝索引。验收时执行 `db.bb_worldcup_daily_task_assign.getIndexes()` 与 `db.bb_worldcup_treasure_roll.getIndexes()`，确认存在：
  - `bb_worldcup_daily_task_assign.idx_wc_daily_assign` UNIQUE（同一用户同日任务分配幂等）
  - `bb_worldcup_treasure_roll.idx_wc_treasure_user_resource` UNIQUE（同一用户同一资源只摇一次）
  - `bb_worldcup_treasure_roll.idx_wc_treasure_user_date`（按用户/日期统计每日中奖上限）
